...
public final class CalculusEngine {
    ...
    public String execute(String command) {
        ...
        CalcObject x =CALC.SYM_EVAL(parsed);
     ...
     * @return The result obtained by parsing and evaluating <b>command</b>.
     */
    public CalcFunction derivative(String command) {
        CalcObject parsed = null;
        try {
            currentTime = System.nanoTime();
            parsed = parser.parse(command);
        ...
        CALC.toggleOperatorNotation();
        return (CalcFunction)CALC.SYM_EVAL(parsed);
    }
    public CalcFunction parser(String command) {
        CalcObject parsed = null;
        try {
            currentTime = System.nanoTime();
            parsed = parser.parse(command);
